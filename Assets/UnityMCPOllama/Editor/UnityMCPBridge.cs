...
public static class UnityMCPBridge
{
    ...
    {
        Debug.Log("[MCP Bridge] Initialized. Ready to execute commands.");
    }
    ...
    public static void ProcessExtractedCommands(JArray commands)
    {
        ...
        {
            Debug.LogWarning("[MCP Bridge] Received no commands to execute.");
            return;
        }

        // --- ДІАГНОСТИЧНИЙ ЛОГ ---
        Debug.Log($"[MCP Bridge] Received a command array with {commands.Count} element(s). Full array: {commands.ToString(Newtonsoft.Json.Formatting.None)}");
        ...
        foreach (JToken cmdToken in commands)
        {
            ...
            if (cmdToken is JObject cmd)
            {
                try
                {
                    ...
                    {
                        Debug.LogWarning("[MCP Bridge] Skipping command with empty function name.");
                        continue;
                    }

                    // --- ДІАГНОСТИЧНИЙ ЛОГ ---
                    Debug.Log($"[MCP Bridge] ==> Executing command: '{function}' with arguments: {arguments.ToString(Newtonsoft.Json.Formatting.None)}");

                    ExecuteCommandByName(function, arguments);
                }
                catch (Exception ex)
                {
                    Debug.LogError($"[MCP Bridge] Error while processing a command object: {ex.Message}\n{ex.StackTrace}");
                }
            }
            else
            {
                // --- ДІАГНОСТИЧНИЙ ЛОГ ---
                Debug.LogWarning($"[MCP Bridge] Skipping an element in the command array because it's not a valid JSON object. Element type: {cmdToken.Type}, Value: {cmdToken.ToString(Newtonsoft.Json.Formatting.None)}");
            }
        }
    }
    ...
    private static void ExecuteCommandByName(string functionName, JObject arguments)
    {
        switch (functionName.ToLower())
        {
            ...
            default:
                Debug.LogWarning($"[MCP Bridge] Unimplemented or unknown command: {functionName}");
                break;
        }
    }
    ...
    private static void CreateObject(JObject args)
    {
        ...
        switch (typeStr)
        {
            ...
            default:
                Debug.LogWarning($"[MCP Bridge] Unknown object type '{typeStr}'. Creating a CUBE instead.");
                obj = GameObject.CreatePrimitive(PrimitiveType.Cube);
                break;
        }

        obj.name = name;
        Debug.Log($"[MCP Bridge] Created '{typeStr}' named '{name}'.");
    ...
    private static void SetObjectTransform(JObject args, GameObject target = null)
    {
        if (target == null)
        {
            ...
            if (target == null)
            {
                Debug.LogError($"[MCP Bridge] Object '{name}' not found in scene for transform operation.");
                return;
            }
        }
        ...
        if (args["position"] is JArray posArray)
        {
            ...
            Debug.Log($"[MCP Bridge] Set position of '{target.name}' to {target.transform.position}.");
        }
        if (args["rotation"] is JArray rotArray)
        {
            target.transform.eulerAngles = ParseVector3(rotArray);
            Debug.Log($"[MCP Bridge] Set rotation of '{target.name}' to {target.transform.eulerAngles}.");
        }
        if (args["scale"] is JArray scaleArray)
        {
            target.transform.localScale = ParseVector3(scaleArray);
            Debug.Log($"[MCP Bridge] Set scale of '{target.name}' to {target.transform.localScale}.");
        }
    }
    ...
    private static void DeleteObject(JObject args)
    {
        ...
        if (string.IsNullOrEmpty(name))
        {
            Debug.LogError("[MCP Bridge] 'name' is required for delete_object.");
            return;
        }
        ...
        if (obj != null)
        {
            ...
            Debug.Log($"[MCP Bridge] Deleted object '{name}'.");
        }
        else
        {
            Debug.LogWarning($"[MCP Bridge] Object '{name}' not found for deletion.");
        }
    }
    ...
    private static void EditorAction(JObject args)
    {
        ...
        switch (action)
        {
            ...
            case "SAVE":
                ...
                Debug.Log("[MCP Bridge] Scene and assets saved.");
                break;
            default: Debug.LogWarning($"[MCP Bridge] Unknown editor action: {action}"); break;
        }
    }
    ...
    private static Vector3 ParseVector3(JArray jArray)
    {
        ...
        {
            Debug.LogWarning($"[MCP Bridge] Failed to parse Vector3 from JArray: {jArray.ToString()}");
            return Vector3.zero;
        }
    }
}